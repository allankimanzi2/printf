...
if (num >= size || num <= -size)
	ret +=
convert_sbase(output, num / size,base, flags, wid -1, prec - 1);
else
{
...
*stores the result to a buffer contained in as struct.
*@output: A buffer_t struct containing a character array.
...
if (num >= size)
	ret +=
convert_ubase(output, num / size, base, flags, wid -1, prec -1);
else
{
