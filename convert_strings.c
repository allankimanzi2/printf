...
*/
unsigned in convert_s(va_list args, buffer_t *output,
		unsigned char flags, int wid, int prec, unsigned char len);
{
char *str, *null = "(null)";
int size;
...
*convert_S - Converts an argument to a string 
*and stores it to a buffer contained in a struct.
*@args: A va_list pointing to the argumen to be convrted.
*@flags: Flag modifiers.                                                                                                                
*@wid: A width modifier.                                                                                                                
*@prec: A precision modifier.                                                                                                           
*@len: A length modifier.                                                                                                               
...
* Return: The number of bytes stored to the buffer.                                                                                   
*
*Description: Non_printable characters (ASCII values < 32 or >= 127)
*are stored as \x followed by the ASCII code vailue in hex.
*/
unsigned int convert_S(va_list args, buffer_t *output,
		unsigned char flags, int wid, int prec, unsigned char len);
{
...
{
ret +=
_memcpy(output, hex, 2);
if (*(str + index) < 16)
ret +=
_memcpy(output, &zero, 1);
ret +=
convert_ubase(output, *(str + index),
...
/**
*convert_r - Converts an argument to a string                                                                                           
*and stores it to a buffer contained in a struct.                                                                                       
*@args: A va_list pointing to the argumen to be convrted.                                                                               
*@flags: Flag modifiers.                                                                                                                
*@wid: A width modifier.                                                                                                                
...
char *rot13 =
"nopgrstuvwxyzabcdefghijklmNOPGRSTUVWXYZABCDEFGHIJKLM';
char *str, *null = "(null)";
int i, j, size;
(void)flags;
(void)len;
